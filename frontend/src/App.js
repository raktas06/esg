...
import { BarChart, Building2, Users, Leaf, Scale, Target, ArrowRight, CheckCircle, AlertCircle, TrendingUp, Download, FileText, PieChart, Activity, Award, Calendar, Globe, DollarSign, Zap, Shield, Calculator, Upload } from "lucide-react";
import { 
  ResponsiveContainer, 
        ...
        {!selectedOrg ? (
          ...
          <Tabs value={currentView} onValueChange={setCurrentView}>
            <TabsList className="grid w-full grid-cols-9 text-xs">
              ...
              <TabsTrigger value="esrs">ESRS Öncel</TabsTrigger>
            ...
            <TabsContent value="assessment" className="space-y-6">
              <div className="text-center mb-8">
                <h2 className="text-3xl font-bold text-gray-900 mb-2">ESG Değerlendirmesi</h2>
                <p className="text-gray-600 max-w-2xl mx-auto">
                  Sürdürülebilirlik performansınızı değerlendirin ve iyileştirme alanlarını belirleyin
                </p>
              </div>

              {/* ESRS Pre-Assessment Section */}
              <Card className="mb-8 border-green-200 bg-green-50">
                <CardHeader>
                  <CardTitle className="flex items-center text-green-800">
                    <CheckCircle className="h-6 w-6 mr-2" />
                    ESRS Ön-Değerlendirmesi (Avrupa Sürdürülebilirlik Raporlama Standartları)
                  </CardTitle>
                  <CardDescription className="text-green-700">
                    ESG raporlaması öncesi şirketinizin mevcut durumunu belirleyin. Her soru için 5 seçenek arasından en uygununu seçin ve otomatik puanlama alın.
                  </CardDescription>
                ...
                <CardContent>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                    <div className="text-center p-4 bg-white rounded-lg border border-green-200">
                      <div className="text-2xl font-bold text-green-600">146</div>
                      <div className="text-sm text-gray-600">ESRS Sorusu</div>
                      <div className="text-xs text-green-600">Kapsamlı Değerlendirme</div>
                    </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                    <div>
                      <h4 className="font-semibold text-green-800 mb-3">🎯 ESRS Değerlendirme Alanları</h4>
                      <ul className="text-sm space-y-2">
                        <li className="flex items-center"><CheckCircle className="h-4 w-4 text-green-600 mr-2" />Hazırlık Esasları ve Kapsam</li>
                        <li className="flex items-center"><CheckCircle className="h-4 w-4 text-green-600 mr-2" />Çevre Performansı (ESRS E)</li>
                        <li className="flex items-center"><CheckCircle className="h-4 w-4 text-green-600 mr-2" />Sosyal Performans (ESRS S)</li>
                        <li className="flex items-center"><CheckCircle className="h-4 w-4 text-green-600 mr-2" />Yönetişim (ESRS G)</li>
                        <li className="flex items-center"><CheckCircle className="h-4 w-4 text-green-600 mr-2" />Finansal Etkiler ve Riskler</li>
                      </ul>
                    </div>
                    <div>
                      <h4 className="font-semibold text-blue-800 mb-3">📊 5 Seviyeli Olgunluk Modeli</h4>
                      <ul className="text-sm space-y-2">
                        <li className="flex items-center"><span className="inline-block w-6 h-4 bg-red-500 rounded mr-2"></span>1. Uygulanmamış</li>
                        <li className="flex items-center"><span className="inline-block w-6 h-4 bg-orange-500 rounded mr-2"></span>2. Zayıf</li>
                        <li className="flex items-center"><span className="inline-block w-6 h-4 bg-yellow-500 rounded mr-2"></span>3. Gelişen</li>
                        <li className="flex items-center"><span className="inline-block w-6 h-4 bg-blue-500 rounded mr-2"></span>4. Güçlü</li>
                        <li className="flex items-center"><span className="inline-block w-6 h-4 bg-green-500 rounded mr-2"></span>5. Rol Model</li>
                      </ul>
                    </div>
                  </div>
                  ...
                  <div className="flex space-x-4">
                    <Button 
                      onClick={async () => {
                        if (!selectedOrg) {
                          alert('Lütfen önce bir organizasyon seçin');
                          return;
                        }
                        ...
                        try {
                          ...
                          setCurrentView('esrs-assessment');
                          
                        } catch (error) {
                          console.error('ESRS assessment error:', error);
                          alert('ESRS değerlendirmesi başlatılamadı: ' + (error.response?.data?.detail || error.message));
                        }
                      }}
                    ...
                    <Button 
                      ...
                      onClick={async () => {
                        ...
                        try {
                          ...
                          alert(`ESRS Değerlendirme Sonuçları:\n\nGenel Skor: ${response.data.overall_score}%\nOlgunluk Seviyesi: ${response.data.maturity_level}\nCevaplanan Sorular: ${response.data.answered_questions}/${response.data.total_questions}\nTamamlanma: %${response.data.completion_percentage}\n\nAna Öneriler:\n${response.data.recommendations.slice(0,3).map(r => `• ${r}`).join('\n')}`);
                          
                        } catch (error) {
                          alert('Sonuçlar yüklenemedi. Önce değerlendirmeyi tamamlayın.');
                        }
                      }}
                    >
                      Sonuçları Görüntüle
                    ...
                    <Button 
                      ...
                      onClick={async () => {
                        ...
                        try {
                          const response = await axios.get(`${API}/esrs/questions/${selectedOrg.id}?limit=5`);
                          
                          let questionText = 'ESRS Örnek Sorular:\n\n';
                          response.data.questions.slice(0,3).forEach((q, i) => {
                            questionText += `${i+1}. ${q.question.question_text.substring(0,80)}...\n`;
                            questionText += `   Cevap Seçenekleri: ${q.answer_options.length} adet (1-5 puan)\n\n`;
                          });
                          
                          alert(questionText + `Toplam ${response.data.total_questions} soru mevcut.`);
                          
                        } catch (error) {
                          alert('Örnek sorular yüklenemedi');
                        }
                      }}
                    >
                      Örnek Soruları Gör
